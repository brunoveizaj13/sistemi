...
@Getter
@Setter
public class OptgInstViewBean implements Serializable {
	...
	public void init() {
		this.institution = login.getUserToken().getUser().getInstitution();
	...
	public void loadPatronageRaport(String nid) {
		try {
			...
			this.selectedPatronage = new PatronageService().findPatronageByNid(nid, IPatronageType.INSTITUTION);
			if (selectedPatronage == null || (selectedPatronage.getInstitution().getId() != this.institution.getId())) {
				...
				patronageForm.setInstitutionId(this.institution.getId());
				personsInPatronage = null;
	...
	public void registerPatronage() {
		try {
			...
			this.selectedPatronage = new PatronageService().registerPatronageInstitution(patronageForm);
	...
	public void addPersonsInPatronage()
	{
		try {
			...
			int nr = new PatronageService().registerPatronageInstitutionPerson(form);
	...
	public void addPersonsInPatronage(PatronagePersonModelForm pp)
	{
		try {
			...
			int nr = new PatronageService().registerPatronageInstitutionPerson(form);
			Messages.throwFacesMessage(nr + " banore u shtuan nen patronazh",1);
			this.personsInPatronage = new PatronageService().getPatronagePersons(this.selectedPatronage.getId(), IPatronageType.INSTITUTION);
		}catch(Exception e)
		{
			Messages.throwFacesMessage(e);
		}
